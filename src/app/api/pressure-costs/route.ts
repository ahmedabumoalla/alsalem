...
export async function DELETE(request: Request) { try { const url = new URL(request.url); const id = url.searchParams.get("id"); if (!id) throw new Error("معرف تكلفة الضغط مطلوب."); await deletePressureCostRecord(id); return jsonData({ deleted: true }); } catch (error) { return jsonError(error); } }
